public class BinarySearch<T extends Comparable<T>> {
    ...
    private Node<T> findHelper(Node<T> node, String key) {
        ...
        int cmp = key.compareTo(((Post)node.data).getAccountName());
        if (cmp < 0) {
    ...
    private Node<T> deleteHelper(Node<T> node, String key) {
        ...
        if (cmp < 0) {
            ...
            if (node.left == null) {
                ...
                node.right = deleteHelper(node.right, ((Post)minRight.data).getAccountName());
            }
        }
        return node;
    }
    ...
    private void inorderHelper(Node<T> node) {
        ...
        System.out.println(((Post)node.data).getAccountName() + ", " + ((Post)node.data).getTittle());
